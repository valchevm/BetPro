...
/* Bottom-sheet modal — footer is FIXED at viewport bottom, body scrolls above it.
   This guarantees footer visibility regardless of bottom nav, viewport quirks, etc.
   Layout:
     - Full-screen overlay (z-50)
     - Sheet container with maxHeight 90vh
     - Footer rendered LAST with fixed positioning inside the sheet
     - Body has paddingBottom equal to footer height */
const BottomSheet = ({ children, footer, onClose, title, icon }) => {
  const [footerHeight, setFooterHeight] = useState(footer ? 80 : 0);
  ...
  // Measure footer height after render so body padding matches
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center" onClick={onClose}>
      ...
      {/* Sheet container */}
      <div className="relative bg-slate-900 border-t border-slate-700 rounded-t-2xl w-full sm:max-w-md sm:mb-4 sm:rounded-2xl sm:border flex flex-col"
           style={{
             maxHeight: "90vh",
             height: "90vh",
             maxWidth: "100vw",
        ...
        {/* Header — fixed at top */}
        <div className="flex-shrink-0 flex items-center justify-between px-4 py-3 border-b border-slate-800 bg-slate-900 rounded-t-2xl">
        ...
        {/* Body — scrollable, takes remaining space, with bottom padding for footer */}
        <div className="flex-1 overflow-y-auto min-h-0 overscroll-contain relative"
             style={{
               WebkitOverflowScrolling: "touch",
               paddingBottom: footer ? `${footerHeight + 16}px` : "max(20px, env(safe-area-inset-bottom, 0px))"
             }}>
          {children}
        </div>

        {/* Footer — absolutely positioned at bottom of sheet, always visible */}
        {footer && (
          <div ref={footerRef}
               className="absolute bottom-0 left-0 right-0 border-t-2 border-slate-700 px-4 py-3 bg-slate-900 rounded-b-2xl"
               style={{
                 paddingBottom: "max(14px, calc(env(safe-area-inset-bottom, 0px) + 14px))",
