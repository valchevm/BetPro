...
import {
  ...
  Coins, ListChecks, PlusCircle, RefreshCw, Receipt, Layers
} from "lucide-react";
...
const T = {
  ...
  compare_books: { bg: "Сравни букмейкъри", en: "Compare bookmakers" },
  bookmakers: { bg: "Букмейкъри", en: "Bookmakers" },
  bookmaker: { bg: "Букмейкър", en: "Bookmaker" },
  primary_book: { bg: "Главен", en: "Primary" },
  no_book_data: { bg: "Няма данни за букмейкъри за този мач.", en: "No bookmaker data for this match." },
  best_odds: { bg: "Най-добри коефициенти", en: "Best odds" },
  na: { bg: "N/A", en: "N/A" },
};
...
/* Extract Bet365 odds from multi_book_odds JSON for primary display.
   Returns object with home/draw/away/over/under/bttsYes/bttsNo (each can be null). */
const extractBet365Odds = (multiBookOdds) => {
  const empty = { home: null, draw: null, away: null, over: null, under: null, bttsYes: null, bttsNo: null };
  if (!multiBookOdds || typeof multiBookOdds !== "object") return empty;
  const bk = multiBookOdds.Bet365;
  if (!Array.isArray(bk)) return empty;
  const result = { ...empty };
  bk.forEach(m => {
    const o = m && m.odds && m.odds[0];
    if (!o) return;
    if (m.name === "ml") {
      result.home = (typeof o.home === "number") ? o.home : null;
      result.draw = (typeof o.draw === "number") ? o.draw : null;
      result.away = (typeof o.away === "number") ? o.away : null;
    } else if (m.name === "ou25") {
      result.over = (typeof o.over === "number") ? o.over : null;
      result.under = (typeof o.under === "number") ? o.under : null;
    } else if (m.name === "btts") {
      result.bttsYes = (typeof o.yes === "number") ? o.yes : null;
      result.bttsNo = (typeof o.no === "number") ? o.no : null;
    }
  });
  return result;
};
...
function normalizeFromOracle(raw) {
  ...
  const multiBookOdds = safeJsonParse(raw.multi_book_odds, null);

  const top_market = raw.top_market || "1";
  const top_prob = Number(raw.top_prob) || 0;

  // Primary odds: Bet365 from multi_book_odds. Null means "not available".
  ...
  const odds = {
    "1": pickOdd(bet365.home, raw.odds_1),
    "X": pickOdd(bet365.draw, raw.odds_x),
    "2": pickOdd(bet365.away, raw.odds_2),
    over: pickOdd(bet365.over, raw.odds_over25),
    under: pickOdd(bet365.under, raw.odds_under25),
    btts_yes: pickOdd(bet365.bttsYes, raw.odds_btts),
    btts_no: pickOdd(bet365.bttsNo, null),
  };
  ...
  return {
    ...
    multiBookOdds, // full parsed multi-bookmaker odds for compare modal
  };
}

// Filter out expired matches.
// A match is considered INACTIVE if either:
//   - It has an explicit expires_at in the past, OR
//   - Its kickoff time was more than MATCH_LINGER_MINUTES ago (default 210 min = 3h 30min)
function isMatchActive(m) {
  const now = Date.now();
  // Hide if kickoff is too far in the past
  if (m.timestamp) {
    try {
      const kickoff = new Date(m.timestamp).getTime();
      if (!Number.isNaN(kickoff) && now > kickoff + MATCH_LINGER_MINUTES * 60 * 1000) {
        return false;
      }
    } catch {}
  }
  // Honor explicit expires_at if Oracle sets it earlier
  if (m.expires_at) {
    try {
...
/* Simple bottom-sheet modal — content rendered inline, no separate footer.
   All action buttons must be placed inside `children` at the end of the form.
   The sheet is lifted above the bottom navigation bar via margin-bottom. */
const BottomSheet = ({ children, onClose, title, icon }) => {
  useEffect(() => {
  ...
  // Height reserved for the bottom navigation bar (icons + labels + safe area)
  const BOTTOM_NAV_HEIGHT = "calc(72px + env(safe-area-inset-bottom, 0px))";
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center" onClick={onClose}>
      ...
      {/* Sheet — lifted above the bottom nav */}
      <div
      ...
      >
        ...
        <div className="flex-shrink-0 flex items-center justify-between px-4 py-3 border-b border-slate-800">
          ...
          <button onClick={onClose} className="text-slate-400 p-1 -mr-1 flex-shrink-0">
            <X size={22} />
          </button>
        </div>
...
// Tappable odd button — adds to bet slip on click. If value is null, shows N/A and is disabled.
  ...
  return (
    <button onClick={disabled ? undefined : onTap}
            ...
            className={`rounded-lg ${compact ? "py-1.5 px-1" : "p-2"} text-center border transition-all relative ${
              disabled ? "bg-slate-800/30 border-slate-800 opacity-50 cursor-not-allowed"
      ...
      {isInSlip && !disabled && <span className="absolute top-0 right-0 w-2 h-2 bg-amber-400 rounded-full" />}
      <div className={`text-[9px] ${disabled ? "text-slate-600" : isInSlip ? "text-amber-300" : "text-slate-400"}`}>{label}</div>
      <div className={`${compact ? "text-sm" : "text-base"} font-bold ${
        disabled ? "text-slate-600" : isInSlip ? "text-amber-300" : isPred ? "text-emerald-400" : "text-white"
      }`}>{disabled ? "N/A" : value.toFixed(2)}</div>
    </button>
  );
};
...
/* Compare odds across bookmakers — table with best odds highlighted green, Bet365 row pinned + highlighted */
const BookmakerCompareModal = ({ match, onClose }) => {
  const t = useT();
  const raw = match.multiBookOdds;
  const markets = [
    { code: "ml",   side: "home",  label: "1" },
    { code: "ml",   side: "draw",  label: "X" },
    { code: "ml",   side: "away",  label: "2" },
    { code: "ou25", side: "over",  label: "O 2.5" },
    { code: "ou25", side: "under", label: "U 2.5" },
    { code: "btts", side: "yes",   label: "BTTS" },
    { code: "btts", side: "no",    label: "B-No" },
  ];

  // Get all bookmaker names — Bet365 always first, rest alphabetical
  const bookNames = raw && typeof raw === "object" ? Object.keys(raw) : [];
  const sortedBooks = [...bookNames].sort((a, b) => {
    if (a === "Bet365") return -1;
    if (b === "Bet365") return 1;
    return a.localeCompare(b);
  });

  // Pick a single odd for a given bookmaker × market × side
  const getOdd = (bk, marketCode, side) => {
    const arr = raw && raw[bk];
    if (!Array.isArray(arr)) return null;
    const mkt = arr.find(x => x && x.name === marketCode);
    const o = mkt && mkt.odds && mkt.odds[0];
    if (!o) return null;
    const v = o[side];
    return typeof v === "number" && v > 0 ? v : null;
  };

  // Best odds per market column (for green highlight)
  const bestByCol = {};
  markets.forEach(m => {
    const key = `${m.code}|${m.side}`;
    let max = 0;
    sortedBooks.forEach(bk => {
      const v = getOdd(bk, m.code, m.side);
      if (v && v > max) max = v;
    });
    bestByCol[key] = max;
  });
  ...
  return (
    <BottomSheet title={t("compare_books")} icon={<Layers size={18} className="text-emerald-400" />} onClose={onClose}>
      <div className="p-3">
        <div className="text-xs text-slate-400 text-center mb-3 truncate">
        ...
        {sortedBooks.length === 0 ? (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center">
            ...
            <div className="text-sm text-slate-400 font-medium">{t("no_book_data")}</div>
          </div>
        ) : (
          <>
            <div className="overflow-x-auto -mx-3 px-3">
              <table className="text-xs border-separate border-spacing-0" style={{ minWidth: "100%" }}>
                <thead>
                  <tr>
                    <th className="sticky left-0 z-10 bg-slate-900 text-left px-2 py-2 text-[10px] uppercase text-slate-400 font-semibold border-b border-slate-700">
                      {t("bookmaker")}
                    </th>
                    {markets.map(m => (
                      <th key={m.label}
                          className="px-1.5 py-2 text-center text-[10px] uppercase text-slate-400 font-semibold border-b border-slate-700"
                          style={{ minWidth: "48px" }}>
                        {m.label}
                      </th>
                    ))}
                ...
                <tbody>
                  {sortedBooks.map(bk => {
                    const isBet365 = bk === "Bet365";
                    return (
                      <tr key={bk} className={isBet365 ? "bg-emerald-500/10" : ""}>
                        <td className={`sticky left-0 z-10 px-2 py-2 font-semibold border-b border-slate-800/40 ${
                          isBet365 ? "bg-emerald-500/10 text-emerald-300" : "bg-slate-900 text-slate-300"
                        }`}>
                          <div className="flex items-center gap-1">
                            <span className="truncate max-w-[80px]">{bk}</span>
                            {isBet365 && <Star size={9} className="text-amber-400 flex-shrink-0" fill="currentColor" />}
                          </div>
                        </td>
                        {markets.map(m => {
                          const v = getOdd(bk, m.code, m.side);
                          const key = `${m.code}|${m.side}`;
                          const isBest = v && v === bestByCol[key] && bestByCol[key] > 0;
                          return (
                            <td key={m.label}
                                className={`px-1.5 py-2 text-center border-b border-slate-800/40 font-mono ${
            ...
            <div className="mt-3 flex items-center justify-between text-[10px] text-slate-500">
              <div className="flex items-center gap-1">
                ...
                <span>{t("primary_book")}: Bet365</span>
              </div>
              <div className="flex items-center gap-1">
                <span className="w-2 h-2 bg-emerald-400 rounded-sm" />
                <span>{t("best_odds")}</span>
              </div>
            </div>
          </>
        )}
...
const MatchPage = ({ match, onBack, onAddToSlip, slipKeys }) => {
  ...
  return (
    <div className="pb-24 relative z-10 w-full">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-3">
              ...
              <div className="grid grid-cols-3 gap-1.5 mb-2">
                {[{ k: "1", lbl: "1", team: match.home }, { k: "X", lbl: "X", team: t("draw") }, { k: "2", lbl: "2", team: match.away }].map((b) => {
                  ...
                  const val = match.odds[b.k];
                  const disabled = val == null;
                  return (
                    <button key={b.k}
                            onClick={disabled ? undefined : () => onAddToSlip(match, b.k, b.k)}
                            disabled={disabled}
                      ...
                      {sel && !disabled && <span className="absolute top-1 right-1 w-2 h-2 bg-amber-400 rounded-full" />}
                      <div className={`text-[10px] ${disabled ? "text-slate-600" : sel ? "text-amber-300" : "text-slate-400"}`}>{b.lbl}</div>
                      <div className={`text-[9px] truncate ${disabled ? "text-slate-700" : sel ? "text-amber-300/70" : "text-slate-500"}`}>{b.team}</div>
                      <div className={`text-lg font-bold mt-0.5 ${disabled ? "text-slate-600" : sel ? "text-amber-300" : ""}`}>
                        {disabled ? "N/A" : val.toFixed(2)}
                      </div>
                      <div className={`text-[10px] ${disabled ? "text-slate-700" : sel ? "text-amber-300/80" : "text-emerald-400"}`}>{match.prob[b.k]}%</div>
                    </button>
                  );
                })}
              </div>
              <div className="grid grid-cols-3 gap-1.5">
                {[{ k: "Over 2.5", oddsKey: "over", probKey: "over", lbl: "Over 2.5" },
                  ...
                  const sel = inSlip(b.k);
                  const val = match.odds[b.oddsKey];
                  const disabled = val == null;
                  return (
                    <button key={b.k}
                            onClick={disabled ? undefined : () => onAddToSlip(match, b.k, b.lbl)}
                            disabled={disabled}
                            className={`rounded-xl p-2 text-center border relative ${
                              disabled ? "bg-slate-800/30 border-slate-800 opacity-50 cursor-not-allowed"
                              : sel ? "bg-amber-500/20 border-amber-400 ring-1 ring-amber-400/50"
                      ...
                      {sel && !disabled && <span className="absolute top-1 right-1 w-2 h-2 bg-amber-400 rounded-full" />}
                      <div className={`text-[10px] ${disabled ? "text-slate-600" : sel ? "text-amber-300" : "text-slate-400"}`}>{b.lbl}</div>
                      <div className={`text-base font-bold mt-0.5 ${disabled ? "text-slate-600" : sel ? "text-amber-300" : ""}`}>
                        {disabled ? "N/A" : val.toFixed(2)}
                      </div>
                      <div className={`text-[10px] ${disabled ? "text-slate-700" : sel ? "text-amber-300/80" : "text-emerald-400"}`}>{match.prob[b.probKey]}%</div>
                    </button>
                  );
                })}
              </div>
              ...
              {/* Compare bookmakers button */}
              <button onClick={() => setShowCompare(true)}
                      className="w-full mt-3 flex items-center justify-center gap-2 bg-slate-800/60 hover:bg-slate-800 border border-slate-700 hover:border-emerald-500/40 text-emerald-400 rounded-lg py-2 text-xs font-bold transition-colors">
                <Layers size={14} />
                <span>{t("compare_books")}</span>
              </button>
            </div>
      ...
      {showCompare && <BookmakerCompareModal match={match} onClose={() => setShowCompare(false)} />}
    </div>
  );
};
...
const BetSlipModal = ({ slip, banks, onClose, onClear, onRemoveLeg, onSubmit }) => {
  ...
  return (
    <BottomSheet title={t("slip")} icon={<Receipt size={18} className="text-amber-400" />} onClose={onClose}>
      <div className="p-3 space-y-3">
        ...
        {/* Inline status messages + action buttons */}
        {savedMsg && (
          <div className="text-center text-emerald-400 text-sm font-bold py-2 bg-emerald-500/10 border border-emerald-500/30 rounded-lg">
            {t("saved_bet")}
          </div>
        )}
        {banks.length === 0 && editedLegs.length > 0 && (
          <div className="text-center text-amber-400 text-xs py-2 bg-amber-500/10 border border-amber-500/30 rounded-lg">
            ⚠ {t("no_bank_first")}
          </div>
        )}
        <div className="pt-3 mt-2 border-t-2 border-emerald-500/30 flex gap-2">
          <button onClick={onClear} disabled={editedLegs.length === 0}
                  className="px-3 py-3.5 bg-red-500/10 border border-red-500/30 text-red-400 rounded-lg text-xs font-semibold disabled:opacity-50">
            <Trash2 size={16} />
          </button>
          <button onClick={onClose} className="flex-1 py-3.5 bg-slate-800 border border-slate-700 rounded-lg text-sm font-medium">{t("cancel")}</button>
          <button onClick={handleSubmit} disabled={!canSave}
                  className="flex-1 py-3.5 bg-emerald-500 text-slate-950 rounded-lg text-sm font-bold disabled:opacity-50 flex items-center justify-center gap-1.5">
...
const CreateBankModal = ({ onClose, onCreate }) => {
  ...
  return (
    <BottomSheet title={t("create_bank")} icon={<Wallet size={18} className="text-emerald-400" />} onClose={onClose}>
      <div className="p-4 space-y-3">
        ...
        {/* Action buttons — inline, always visible at end of form */}
        <div className="pt-4 mt-2 border-t-2 border-emerald-500/30 flex gap-2">
          <button onClick={onClose}
                  className="flex-1 py-3.5 bg-slate-800 border border-slate-700 rounded-lg text-sm font-medium">
            {t("cancel")}
          </button>
...
const EditBetModal = ({ bet, onClose, onSave }) => {
  ...
  return (
    <BottomSheet title={t("edit_bet")} icon={<Edit2 size={18} className="text-emerald-400" />} onClose={onClose}>
      <div className="p-3 space-y-2">
        ...
        {/* Action buttons — inline at end of content */}
        <div className="pt-4 mt-2 border-t-2 border-emerald-500/30 flex gap-2">
          <button onClick={onClose}
                  className="flex-1 py-3.5 bg-slate-800 border border-slate-700 rounded-lg text-sm font-medium">
            {t("cancel")}
...
export default function App() {
  ...
  const [sharpSignal, setSharpSignal] = useState(null);

  // Persist state to localStorage on every change
  useEffect(() => { saveState(state); }, [state]);

  // Listen for sharp-drop signals from xAPEX (or test triggers from devtools)
  // To test from browser console:
  //   window.dispatchEvent(new CustomEvent('sharp-drop-signal'))            // uses default test data
  //   window.dispatchEvent(new CustomEvent('sharp-drop-signal',{detail:{...}})) // real signal
  useEffect(() => {
    if (typeof window === "undefined") return;
    const handler = (e) => setSharpSignal(e.detail || {});
    window.addEventListener("sharp-drop-signal", handler);
    return () => window.removeEventListener("sharp-drop-signal", handler);
  }, []);
  ...
  const handleAddToSlip = (m, market, label) => {
    ...
    // Don't add markets with no Bet365 price available
  ...
  // Add a leg from the SharpDropModal — accepts a pre-built leg + close the modal
  const handleAddSharpDropToSlip = (leg) => {
    setState(prev => ({
      ...prev,
      betSlip: [...(prev.betSlip || []), {
        matchId: leg.matchId,
        home: leg.home,
        away: leg.away,
        league: leg.league,
  ...
  return (
    <LangCtx.Provider value={{ lang: state.lang, setLang }}>
      <div className="min-h-screen w-full bg-slate-950 text-white relative overflow-x-hidden"
        ...
        {sharpSignal && (
          <SharpDropModal
            signal={Object.keys(sharpSignal).length > 0 ? sharpSignal : undefined}
            books={sharpSignal.books || undefined}
            bankroll={state.banks?.[0]?.balance ?? 100}
            currency={state.banks?.[0]?.currency === "EUR" ? "€" : "u"}
            onClose={() => setSharpSignal(null)}
            onAddToSlip={handleAddSharpDropToSlip}
          />
        )}
      </div>
    </LangCtx.Provider>
